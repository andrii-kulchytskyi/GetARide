...
//                FancyToast.makeText(ViewLocationMapActivity.this, getIntent().getStringExtra("rUsername"), Toast.LENGTH_SHORT, FancyToast.SUCCESS, true).show();
                    ...
                    public void done(List<ParseObject> objects, ParseException e) {
                        if (objects.size() > 0 && e == null) {
                            for (ParseObject carRequest : objects) {
                                carRequest.put("driverOfMe", ParseUser.getCurrentUser().getUsername());
                                carRequest.saveInBackground(new SaveCallback() {
                                    @Override
